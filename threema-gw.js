...
adapter.on('unload', function (callback) {
    try {
        adapter.setState('info.connection', {val: false, ack: true}); //connection to Threema gateway not established
        adapter.setState('info.credits', {val: -999, ack: true});    //set credits at Threema gateway to a defaul value (-999 = 'unknown')
        adapter.setState('info.lastresponse', {val: 'adapter is not running', ack: true});
        ...
        ThreemaRequest(ThreemaGatewayCall, function (error, response, result) {
            switch(response.statusCode){
                case 200:
                    ...
                    adapter.setState('info.credits', {val: result, ack: true});    //amount of credits at Threema gateway
                    adapter.setState('info.connection', {val: true, ack: true});  //connection to Threema gateway established
                    break;
                case 401:
                    responseFromAPI = '401 - API identity or secret are incorrect';
                    adapter.setState('info.credits', {val: -999, ack: true});    //set credits at Threema gateway to a defaul value (-999 = 'unknown')
                    adapter.setState('info.connection', {val: false, ack: true});  //connection to Threema gateway established
                    break;
                case 402:
                    aresponseFromAPI = '402 - no credits remain';
                    adapter.setState('info.credits', {val: 0, ack: true});    //amount of credits at Threema gateway
                    adapter.setState('info.connection', {val: true, ack: true});  //connection to Threema gateway established
                    break;
                case 404:
            ...
            if (firstTime) {adapter.setState('info.lastresponse', {val: responseFromAPI, ack: true})};
...
function SendThreemaSimpleMessage(obj){
    ...
    try {
        ThreemaRequest.post({url:ThreemaGatewaySendSimple, form: formdata}, function (error, response, result) {
            ...
            adapter.log.info(responseFromAPI);
            adapter.setState('info.lastresponse', {val: responseFromAPI, ack: true});
